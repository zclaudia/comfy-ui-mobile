...
export function ChatComposer({ value, onChange, disabled, placeholder, attachments, onAddFiles, onRemoveAttachment, onRetryAttachment, canSend, onSend }: {
  value: string; onChange: (value: string) => void; disabled: boolean; placeholder: string;
  attachments: LocalAttachment[]; onAddFiles: (files: File[]) => void; onRemoveAttachment: (id: string) => void; onRetryAttachment: (id: string) => void;
  ...
  return <form data-agent-composer className={`rounded-[22px] border transition-colors ${disabled ? 'border-white/[0.06] opacity-60' : 'border-white/[0.1] focus-within:border-[#3069f0]/60 focus-within:shadow-[0_0_0_3px_rgba(48,105,240,0.15)]'}`} style={{ background: '#14161c' }}
    ...
    <div className="flex items-end gap-1.5 pl-2 pr-2 py-2">
      ...
      <textarea ref={textarea} aria-label={at('给助手的消息')} value={value} onChange={e => onChange(e.target.value)} onKeyDown={onKeyDown} onPaste={onPaste} rows={1} maxLength={MAX_LENGTH} disabled={disabled} placeholder={placeholder}
