    ...
    {items.map(item => <li key={item.id} data-attachment-status={item.status} className={`relative shrink-0 rounded-[12px] border overflow-hidden ${item.status === 'error' ? 'border-[#f25555]/60' : 'border-white/[0.1]'} ${item.kind === 'image' ? 'w-[68px] h-[68px]' : 'h-[68px] w-[150px]'}`} style={{ background: '#15181e' }}>
      {item.kind === 'image' && item.previewUrl
        ? item.previewUrl.startsWith('blob:')
          ...
          : <AuthenticatedImage source={item.previewUrl} alt={item.uploaded?.name ?? ''} className="w-full h-full object-cover" />
        : <div className="h-full flex items-center gap-2 px-2.5 text-[#c8ccd4]">{iconFor(item.kind)}<div className="min-w-0"><p className="text-[11.5px] font-medium truncate">{item.file?.name ?? item.uploaded?.name ?? ''}</p><p className="font-mono text-[9.5px] text-[#66758a]">{item.file ? formatBytes(item.file.size) : ''}{item.uploaded?.width && item.uploaded.height ? ` · ${item.uploaded.width}×${item.uploaded.height}` : ''}</p></div></div>}
