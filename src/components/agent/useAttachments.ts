...
export async function uploadAttachment(baseUrl: string, file: File, signal?: AbortSignal): Promise<AgentAttachment> {
  ...
  return { filename: result.name, subfolder: result.subfolder ?? ATTACHMENT_SUBFOLDER, type: result.type === 'temp' ? 'temp' : 'input', kind: attachmentKind(file), name: file.name, size: file.size };
}
