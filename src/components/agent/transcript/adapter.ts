...
export function buildTranscript(events: AgentEvent[], tasks: AgentTask[] = [], caughtUp = false): TranscriptState {
  ...
  for (const event of [...events].sort((a, b) => a.seq - b.seq)) {
    ...
    } else if (['workflow', 'result', 'execution_error', 'saved', 'context', 'approval', 'retry', 'draft_created', 'revision_created', 'run_state', 'selection_requested', 'library_save_state'].includes(kind)) {
