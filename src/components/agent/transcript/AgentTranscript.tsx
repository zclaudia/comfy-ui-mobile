...
import type { WorkspaceTask } from '../../../shared/types/agentWorkspace';
...
const names: Record<string, string> = {
  ...
  list_session_outputs: '查看对话生成结果', prepare_output_image: '准备生成图片作为参考',
  ...
  plan_operations: '安排本轮操作', list_drafts: '查找创作', list_versions: '查看版本历史', list_assets: '查找参考素材', get_asset: '查看素材来源', list_runs: '查看生成记录',
  edit_workflow: '调整工作流', fork_workflow: '创建另一创作方向', replace_workflow_template: '更换工作流模板', restore_workflow: '恢复工作流版本', request_selection: '等待选择',
};
...
export function AgentTranscript({ events, tasks, caughtUp, renderContent, renderUserContext, renderUserAttachments, baseUrl = '' }: {
  events: AgentEvent[]; tasks: AgentTask[]; caughtUp: boolean; renderContent: (event: AgentEvent) => ReactNode; baseUrl?: string;
  renderUserContext?: (event: AgentEvent) => ReactNode;
  renderUserAttachments?: (event: AgentEvent) => ReactNode;
}) {
  ...
  const userEvents = useMemo(() => new Map(events.filter(e => e.kind === 'user').map(e => [`event:${e.seq}`, e])), [events]);
  return <TranscriptCapabilitiesProvider value={capabilities}><section aria-label={at('对话记录')} className="agent-transcript space-y-4">
    {transcript.items.map(item => {
      if (item.kind === 'user_message') {
        ...
        return <article key={item.id} className="ml-8 rounded-2xl bg-blue-600/20 border border-blue-500/20 p-4 text-sm"><p className="text-xs text-slate-500 mb-1">{at('你')}</p>{renderUserAttachments && userEvents.has(item.id) ? renderUserAttachments(userEvents.get(item.id)!) : sent.length > 0 && <SentAttachments attachments={sent} baseUrl={baseUrl} />}{renderUserContext && userEvents.has(item.id) && renderUserContext(userEvents.get(item.id)!)}{item.text && <div className="whitespace-pre-wrap break-words">{item.text}</div>}</article>;
      }
      ...
      return <article key={item.id} data-agent-turn={item.id} className="min-w-0 rounded-2xl border border-white/5 bg-white/[0.035] p-3 text-sm leading-7">
        ...
        <div className="space-y-3">{item.blocks.map((block, index) => {
          ...
          if (!call) return null;
          const resultState = call.result && typeof call.result === 'object' ? (call.result as { state?: string; status?: string }).state ?? (call.result as { status?: string }).status : undefined;
          const held = call.name === 'submit_preview' && ['awaiting_user', 'awaiting_approval'].includes(resultState ?? '');
          const summary = call.status === 'running' ? '执行中' : call.status === 'error' ? '未完成' : call.status === 'cancelled' ? '已停止等待' : held ? '等待你确认' : call.name === 'request_selection' && resultState === 'pending' ? '等待选择' : call.name === 'submit_preview' ? ['failed', 'cancelled', 'unknown'].includes(resultState ?? '') ? '未完成' : '已提交，生成结果见下方' : '已完成';
        ...
        <p className="text-xs text-slate-500 mt-3" data-turn-status={item.status}>{at(item.status === 'complete' ? '本轮完成' : item.status === 'cancelled' ? '助手已停止；已提交的生成可能继续运行' : item.status === 'failed' ? '本轮未完成' : (task as WorkspaceTask | undefined)?.workspace?.waitingReason?.type === 'selection' ? '等待选择' : statuses[task?.state ?? ''] ?? '正在处理')}</p>
      </article>;
    })}
