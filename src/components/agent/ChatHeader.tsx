...
export function ChatHeader({ title, subtitle, onBack, onOpenCanvas, onRename, onHistory, onDelete, onSaveToLibrary, confirmPreviews, onToggleConfirmPreviews }: {
  ...
  confirmPreviews?: boolean; onToggleConfirmPreviews?: () => void; onSaveToLibrary?: () => void;
}) {
  ...
  return <header className="shrink-0 z-10 border-b border-white/[0.08] pwa-header" style={{ background: 'rgba(11,12,15,0.86)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
    <div className="h-14 flex items-center gap-[11px] px-3">
      ...
      {(onRename || onHistory || onDelete || onToggleConfirmPreviews || onSaveToLibrary) && <Dropdown.Root>
        ...
        <Dropdown.Portal><Dropdown.Content align="end" sideOffset={6} className="z-[60] min-w-[180px] p-1 rounded-[12px] border border-white/[0.08] shadow-2xl" style={{ background: '#101217' }}>
          ...
          {onSaveToLibrary && <Dropdown.Item className={item} onSelect={() => setTimeout(onSaveToLibrary, 0)}><Save size={15} />{at('保存到工作流库')}</Dropdown.Item>}
