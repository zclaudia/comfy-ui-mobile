...
export function AgentMedia({baseUrl,output,index}:{baseUrl:string;output:AgentMediaOutput;index:number}) {
  ...
  return <figure className="min-w-0 space-y-2" data-agent-media={kind}>
    ...
    {media.url && (kind==='video'?<video controls playsInline preload="metadata" src={media.url} className="w-full rounded-lg max-h-[500px]" aria-label={at('生成视频 {{index}}', { index: index + 1 })} />:<audio controls preload="metadata" src={media.url} className="w-full" aria-label={at('生成音频 {{index}}', { index: index + 1 })} />)}
