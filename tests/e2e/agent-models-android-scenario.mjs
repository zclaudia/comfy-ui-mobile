...
export async function agentModelsAndroidScenario({app,waitFor,assert,adb,sleep}) {
  ...
  for(const profile of ['z-image-turbo','h3-fl2va-lite']) {
    ...
    // The composer renders the send button asynchronously after the status
    // probe settles; clicking before it exists loses the message silently.
    await waitFor(`!!document.querySelector('button[aria-label="发送消息"]:not([disabled])')`,15000);
    ...
    const outcome=await waitFor(`(()=>{const error=document.querySelector('[role=alert]');if(error)return {error:error.textContent};return !!document.querySelector('[data-agent-card="result"]')&&document.body.innerText.includes('已保存')&&!document.querySelector('footer [role=status]')?{done:true}:null})()`,900000);
    ...
    if(profile==='z-image-turbo') {
      ...
      const metadata=await waitFor(`(()=>{const v=document.querySelector('[data-agent-media=video] video');return v&&v.readyState>=2&&v.duration>0?{duration:v.duration,width:v.videoWidth,height:v.videoHeight}:null})()`,90000);
    ...
    // Drafts never reach the library implicitly: save explicitly, then the
    // header offers the canvas entry for the saved workflow.
    await waitFor(`!!document.querySelector('[data-agent-save-library]:not([disabled])')`,30000);
    ...
    await waitFor(`!!document.querySelector('[data-agent-open-canvas]')`,30000);
