...
export async function agentModelsAndroidScenario({app,waitFor,assert,adb,sleep}) {
  ...
  for(const profile of ['z-image-turbo','h3-fl2va-lite']) {
    ...
    await waitFor(`!!document.querySelector('select[aria-label="选择会话"]')`,20000);
    const previous=await app.evaluate(`document.querySelector('select[aria-label="选择会话"]').value`);
    await app.evaluate(`[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='新建').click();true`);
    await waitFor(`document.querySelector('select[aria-label="选择会话"]')?.value!==${JSON.stringify(previous)} && !!document.querySelector('textarea:not(:disabled)')`,20000);
    ...
    const outcome=await waitFor(`(()=>{const error=document.querySelector('[role=alert]');if(error)return {error:error.textContent};return document.body.innerText.includes('生成结果 · v')&&document.body.innerText.includes('已保存')&&!document.querySelector('footer [role=status]')?{done:true}:null})()`,420000);
