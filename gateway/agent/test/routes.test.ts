...
test('real Gateway agent routes enforce auth, cross-device session sharing, payload validation and version access', async t => {
  ...
  const boundCreate = await request('/agent/sessions', { name: '海报会话', canvas, sourceRef: source });
  assert.equal(boundCreate.status, 201);
  const bound = (await boundCreate.json()).session;
  assert.equal(bound.name, '海报会话');
  assert.equal(bound.workspaceMode, 'draft');
  assert.equal((await request('/agent/sessions', { name: 'x', workflow: { id: 'wf-1', name: '海报' } })).status, 400, 'the old binding field is gone');
  const listed = (await (await request('/agent/sessions')).json()).sessions.find((s: any) => s.id === bound.id);
  assert.deepEqual(listed.sourceRef, source);
  assert.equal(listed.active, false);
  assert.equal(typeof listed.lastActivity, 'number');
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { sourceRef: null }, device)).status, 200);
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { owner: 'x' })).status, 400);
  const patched = await call('PATCH', `/agent/sessions/${bound.id}`, { name: '新名字', sourceRef: null });
  assert.equal(patched.status, 200);
  assert.equal((await patched.json()).session.sourceRef, undefined);
  const hash = 'c'.repeat(64);
  const op = { opId: 'op-1', mode: 'create', draftVersion: 1, graphHash: hash, target: { serverId: source.serverId, workflowId: 'wf-9', filename: '新.json', name: '新' }, state: 'pending', startedBy: 'browser', startedAt: 1, updatedAt: 1 };
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { librarySaveOp: op })).status, 200);
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { librarySaveOp: { ...op, opId: 'op-2' } }, device)).status, 409, 'another device sees the in-flight save');
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { librarySaveOp: { ...op, graphHash: 'nope' } })).status, 400);
  assert.equal((await call('PATCH', `/agent/sessions/${bound.id}`, { librarySaveOp: { ...op, state: 'applying', updatedAt: 2 } })).status, 200);
  const finished = await call('PATCH', `/agent/sessions/${bound.id}`, { librarySaveOp: { ...op, state: 'succeeded', updatedAt: 3, result: { etag: 'e9' } }, lastLibrarySave: { ...source, workflowId: 'wf-9', filename: '新.json', name: '新', draftVersion: 1, graphHash: hash, etag: 'e9', opId: 'op-1', at: 3 } });
  assert.equal(finished.status, 200);
  assert.equal((await finished.json()).session.lastLibrarySave.etag, 'e9');
  const requestId = randomUUID();
  const imported = await call('POST', `/agent/sessions/${bound.id}/versions`, { canvas, baseVersion: 1, summary: '画布修改', requestId });
  assert.equal(imported.status, 200);
  assert.equal((await imported.json()).version, 2);
  assert.equal((await (await call('POST', `/agent/sessions/${bound.id}/versions`, { canvas, baseVersion: 1, summary: '画布修改', requestId })).json()).version, 2, 'retry replays');
  const page = await call('GET', `/agent/sessions/${bound.id}/versions?limit=1`);
  assert.equal(page.status, 200);
  assert.deepEqual((await page.json()).versions.map((v: any) => v.version), [2]);
  assert.equal((await (await call('GET', `/agent/sessions/${bound.id}/versions?before=2&limit=1`)).json()).hasMore, false);
  assert.equal((await call('GET', `/agent/sessions/${bound.id}/versions?limit=0`)).status, 400);
