...
export async function compactContext(options: {
  ...
  /** Each summary is a separate provider request; cancellation/deadline still bound every request. */
  requestSignal?: () => AbortSignal;
  /** Summary calls omit the action tools/state, so their request can use this larger budget. */
  summaryBudget?: number;
  /** Multiplier calibrated from the provider's reported input tokens (see AgentService); 1 keeps the raw estimate. */
  scale?: number;
  ...
  if (estimate(options.messages) <= budget) return { messages: options.messages, compacted: false };
  ...
  for (const group of groups) {
    if (estimate(groups.filter(g => retained.has(g)).flat()) <= target - 1200) break;
  ...
  if (estimate(suffix) > budget - 1200) throw new ContextError('当前消息或附件超出模型上下文，请缩短消息、减少图片或增大上下文窗口');
  ...
  while (offset < chars.length) {
    ...
    const requestBudget = options.summaryBudget ?? budget;
    const chunkBudget = Math.floor(requestBudget * 0.85) - estimate(summarySystem)
      - estimate({ previousMemory: memory, excerpt: '' }) - outputLimit - 256;
    if (chunkBudget < 128) throw new ContextError('摘要请求上下文不足，请增大模型上下文窗口');
    let end = Math.min(chars.length, offset + chunkBudget * 2);
    while (end > offset + 1 && estimate(chars.slice(offset, end).join('')) > chunkBudget) end = offset + Math.floor((end - offset) * 0.75);
    const excerpt = chars.slice(offset, end).join('');
    const requestSignal = options.requestSignal?.() ?? signal;
    ...
    const result = await generateText({ model, system: summarySystem,
      ...
      maxOutputTokens: outputLimit, maxRetries: 0, abortSignal: requestSignal,
    });
    requestSignal.throwIfAborted();
    ...
    if (estimate(memory) > Math.max(1600, budget / 3)) throw new ContextError('模型返回的摘要过长，请检查模型输出上限后重试');
  ...
  if (estimate(messages) > budget) throw new ContextError('压缩后上下文仍然过大，请增大模型上下文窗口');
